...
def encrypt_text(input_file, output_file, shift1, shift2):
    """
    Reads input_file, applies the shift rules, and writes to output_file.
    """
    try:
...
def decrypt_text(input_file, output_file, shift1, shift2):
    """
    Reads encrypted file, reverses logic based on valid ranges, writes to output_file.
    """
    try:
        with open(input_file, 'r') as f:
            content = f.read()
...
def verify_decryption(original_file, decrypted_file):
    """
    Compares the two files and prints the result.
    """
    try:
...
if __name__ == "__main__":
    # Create a dummy file if it doesn't exist so the code runs immediately
